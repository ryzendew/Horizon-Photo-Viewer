...
// ── M3-Style Slider ─────────────────────────────────────────────────────────
// Self-contained: no external deps beyond Cairo.  Supports continuous/discrete
// drag, grab-offset (no thumb jump), value popup during drag, active/inactive
// track, stop indicator dot, and thumb scale-on-press.
class M3Slider {
public:
    ...
    bool handlePointerDown(float px, float py) {
        ...
        float dist = px - tc;
        // Maintain grab offset when clicking on thumb so it doesn't jump;
        // when clicking on track, snap thumb to pointer (offset = 0).
